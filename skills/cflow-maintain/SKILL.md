...
5. **泛化检查**：禁止把失败案例里的具体平台、格式、对象、行业或文案场景直接固化为规则条件。先做同类替换测试，能跨场景成立的必须写成可迁移规则；原案例只作为测试用例或例子。
6. **反膨胀审计**：默认不新增规则。先判断能否已覆盖、合并、移动、删除、收紧、作为例子、作为测试用例、分流到 profile/用户资产，或拒绝沉淀。
7. **特殊情况分流**：单次案例、作者偏好、特定业务资产、平台短期玩法、外部方法论整包或已有规则未执行，都先分流处理，不直接写进通用 skill。
