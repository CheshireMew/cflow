...
`cflow-marketing` 负责转化策略。offer、funnel stage、CTA 类型、CTA 强度、主转化路径和异议处理都要收口到可执行表达；如果用户只要“这句话怎么写得更吸引人 / CTA 怎么表达”，且 offer 和转化路径已稳定，直接打磨表达。
...
8. **交付生产假设**：输出正文、标题、hook、opening、短发布文案或 CTA 文案变体需要遵守的转化假设；发布后转化数据进入复盘。
