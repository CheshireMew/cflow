---
name: cflow-package
description: CFlow 包装系统。用于为接近完成的内容创建 title、hook、opening、subtitle、CTA、cover text、caption、thumbnail text、平台版本和发布包。触发场景包括：起标题、优化开头、短视频前三秒、小红书/RED 封面、LinkedIn headline、Twitter/X thread first post、newsletter/blog 标题、平台适配、提高点击和停留。
---

# CFlow Package

## 边界

只负责呈现和平台适配。除非包装过程暴露出核心主张不清，否则不要改内容主张；如果发现问题，只提出最小修法。硬广/软广、offer、funnel stage 和转化路径归 `$cflow-marketing`。

包装应该放大内容真实价值，不制造假刺激。

## 工作流

1. 明确平台、读者、核心主张和希望读者采取的行动。
2. 从内容中提取最可点击但真实的张力。
3. 按类型生成包装选项：标题、hook、封面文案、CTA 或平台版本。
4. 选项要策略不同，不要只是同义改写。
5. 标出最佳选项，并简要说明选择标准。
6. 如果 CTA 强度、offer 或转化路径不清，先交给 `$cflow-marketing`。
7. 发布后有数据时交给 `$cflow-review`。

## 输出

标题或 hook 请求：

- 最佳选项
- 备选项
- 为什么最佳
- 需要避免的风险

发布包请求：

- 标题或 headline
- 开头或 hook
- 必要的正文框架
- CTA
- 平台说明

## 参考

当需要跨平台生成变体或避免 clickbait 时，读取 `references/package-patterns.md`。
