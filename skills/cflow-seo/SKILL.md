...
2. **判断是否需要数据**：默认基于用户素材做结构和点击优化；只有用户要求来源包、实时 SERP 或事实核查时才交给 `$cflow-research`。
...
5. **设计可发现内容结构**：给出 H1/H2、摘要、定义、步骤、比较、FAQ、可引用段落和结论前置。
6. **优化 AI 可引用性**：让内容包含清楚定义、列表、表格、例子、判断条件和简洁答案；用户要求严谨时再补来源和事实边界。
...
未联网验证的信息不能写成“已核查事实”。但普通结构优化不需要输出大段未联网提示，除非用户要求严谨来源或当前任务就是事实判断。
