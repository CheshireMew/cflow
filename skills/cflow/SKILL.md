...
所有会生产、改写、包装或迁移文字的专项 skill，都受 `references/content-production-contract.md` 约束。专项 reference 只维护本场景的应用规则，不重复维护通用写作纪律。
...
用户指出“不是这个意思”“不吸引人”“很平淡”“没用技巧”“总觉得不对”“怪”“像 AI 写的”“不像我”或类似反馈时，不要继续在同一策略里改句子。先判断纠偏指向哪个层级：

- **指代错误**：重新识别主对象、主链接、主 CTA。
- **路由错误**：切到更合适的专项 skill 或组合链路。
- **素材越界**：删掉没有素材支撑的新增内容。
- **策略无效**：换 hook 机制、营销形态或内容结构。
- **声音不对**：交给 `$cflow-voice` 或降低改写强度，沉淀作者禁区和外来腔信号。
- **翻译边界错误**：区分直译、润色、本地化和重写；没有用户授权时，不把翻译自动改成平台爆款再创作。

如果用户指出“平淡”“没技巧”“不吸引人”，默认不是句子润色问题，而是包装策略失败；交给 `$cflow-package` 用不同策略重做候选。

如果用户指出“像 AI”“不像人”“不像我”，默认不是再生成更多版本的问题。先交给 `$cflow-edit` 或 `$cflow-voice` 做根因诊断：哪些句子太满、太顺、太对称，哪些表达像外来腔，哪些强原句和毛边应该保留。只有用户要求修改或诊断后明确需要改稿时，再进入重写。
