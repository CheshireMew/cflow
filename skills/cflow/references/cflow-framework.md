...
| 独立传播方案 | 是否需要脱离短内容本体，单独设计传播路径、二创机制、跨环境扩散或传播风险 | `$cflow-viral` |
...
生产完成后，继续判断是否需要发布前质检、包装或沉淀：能不能发、哪里会失败和下一刀交给 `$cflow-check`；AI 味、结构问题和声音问题由 `$cflow-check` 路由到 `$cflow-edit` 或 `$cflow-voice`；标题、hook、CTA 和短发布文案交给 `$cflow-package`；搜索结构交给 `$cflow-seo`；独立传播路径、二创机制、跨环境扩散或传播风险交给 `$cflow-viral`；发布后表现学习交给 `$cflow-review`；如果复盘发现可迁移生产机制，交给 `$cflow-absorb`；如果发现重复、冲突、旧架构或唯一事实源问题，再交给 `$cflow-maintain`。
...
- **素材越界**：删掉没有素材支撑的新增内容。
- **策略无效**：先判断当前生产 skill 是否本来负责该质量，再换入口、hook 机制、营销形态或内容结构。
- **声音不对**：交给 `$cflow-voice` 或降低改写强度。

如果用户指出“平淡”“没技巧”“不吸引人”，默认不是句子润色问题。先判断当前任务的交付物：

- 如果正在写短帖、短脚本、短发布文案或短内容系列，首屏张力、单点吸引力和推进链路属于 `$cflow-shortform` 的本体质量，先在 shortform 内重做。
- 如果任务是标题、hook、opening、CTA、发布包或普通包装资产，交给 `$cflow-package` 用不同策略重做候选。
- 如果正在写短帖、短脚本、短发布文案或短内容系列，分享动机、截图点、复述句、评论入口和基础二创点属于 `$cflow-shortform` 的本体质量，先在 shortform 内重做。

用户点名某个 skill 时，先判断它是执行协议还是内容对象。不得把 `$cflow-shortform`、`$cflow-package`、README、文件路径或 skill 名称自动当成正文要推广的对象；连续纠偏一次后，必须重新确认主对象、执行 skill 和交付形态。
